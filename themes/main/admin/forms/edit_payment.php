<?php
$model = \app\models\Fizetes::findOne($id) ?: new \App\Models\Fizetes();

?>

<form data-ajax-form data-action="/admin-api/edit?class=Fizetes&id=<?=$id?>" data-redirect-url="(current)">

    <?=\app\components\Helpers::render('ui/input', [
        'name' => 'nev',
        'label' => 'Név',
        'type' => 'text',
        'value' => $model->nev,
    ])?>

    <?=\app\components\Helpers::render('ui/input', [
        'name' => 'megnevezes_szamlan',
        'label' => 'Megnevezés a számlán',
        'type' => 'text',
        'placeholder' => 'Opcionális',
        'tooltip' => 'Ilyen néven fog szerepelni a számlán. Üresen hagyva a "Név" mező értékét veszi fel',
        'value' => $model->megnevezes_szamlan,
    ])?>


    <?=\app\components\Helpers::render('ui/textarea', [
        'name' => 'leiras',
        'label' => 'Leírás',
        'rows' => 2,
        'type' => 'text',
        'value' => $model->leiras,
    ])?>

    <?=\app\components\Helpers::render('ui/textarea', [
        'name' => 'fizetesi_instrukcio',
        'label' => 'Fizetési instrukció',
        'type' => 'text',
        'value' => $model->fizetesi_instrukcio,
    ])?>

    <?=\app\components\Helpers::render('ui/input', [
        'name' => 'ar',
        'label' => 'Ár',
        'icon' => 'Ft',
        'type' => 'text',
        'value' => $model->ar,
    ])?>

    <?=\app\components\Helpers::render('ui/select', [
        'name' => 'szolgaltato',
        'label' => 'Szolgáltató',
        'values' => [
            '' => '(nincs)',
            'stripe' => 'Stripe',
        ],
        'value' => $model->szolgaltato,
    ])?>

    <?=\app\components\Helpers::render('ui/toggle', [
        'name' => 'utanvet',
        'label' => 'Utánvét',
        'value' => $model->utanvet ?: 0,
    ])?>

    <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
        <button type="button" data-close class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:col-start-1 sm:text-sm">
            Mégse
        </button>
        <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:col-start-2 sm:text-sm">
            Mentés
        </button>

    </div>

</form>

