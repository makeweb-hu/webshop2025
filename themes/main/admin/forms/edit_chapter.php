<?php
$model = \app\models\TudastarFejezet::findOne($id) ?: new \App\Models\TudastarFejezet();
$article = \app\models\Tudastar::findOne($article_id ?? $model->article->id);
?>

<form data-ajax-form data-action="/admin-api/edit?class=TudastarFejezet&id=<?=$id?>" data-redirect-url="(current)">
    <input type="hidden" name="tudastar_id" value="<?=$article->id?>">
