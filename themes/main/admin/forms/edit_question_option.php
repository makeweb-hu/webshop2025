<?php
$model = \app\models\KerdoivKerdesFelelet::findOne($id) ?: new \app\models\KerdoivKerdesFelelet();

?>

<form data-ajax-form data-action="/admin-api/edit?class=KerdoivKerdesFelelet&id=<?=$id?>" data-redirect-url="(current)">
    <?=\app\components\Helpers::render('ui/langs', [

    ])?>

    <input type="hidden" name="felelet" value="deprecated">

    <?php foreach (['hu','de','en','pl','fr','bg'] as $lang): $name = 'felelet_' . $lang; ?>
