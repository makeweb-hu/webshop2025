...
<form data-ajax-form data-action="/admin-api/edit?class=Kosar&id=<?=$id?>" data-redirect-url="(current)">
    <?=\app\components\Helpers::render('ui/select', [
        'label' => 'Teljesítés státusza',
        'name' => 'statusz',
        'value' => $model->statusz,
        'values' => \app\models\Kosar::statuses(),
    ])?>

    <div class="flex items-center" data-visible-if="statusz:atveheto,kiszallitas_alatt,elvetve,adatok_modositva">
        <input id="sendEmail"  name="sendEmail" type="checkbox" class="h-4 w-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500">
        <div class="ml-2">
            <label for="sendEmail" class="text-sm  text-gray-900">E-mail küldése a vásárlónak.</label>
        </div>
    </div>
