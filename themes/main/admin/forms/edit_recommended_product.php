<?php
$model = \app\models\TermekAjanlo::findOne($id) ?: new \App\Models\TermekAjanlo();

?>

<form data-ajax-form data-action="/admin-api/edit?class=TermekAjanlo&id=<?=$id?>" data-redirect-url="(current)" >

    <input type="hidden" name="termek_id" value="<?=$product_id?>">

    <?=\app\components\Helpers::render('ui/lookup', [
        'name' => 'masik_termek_id',
        'value' => $model->masik_termek_id,
