...
use app\components\Stripe\StripeObject;
...
abstract class Util
{
    ...
    {
        $types = \app\components\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \app\components\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \app\components\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
