...
class OAuthService extends \app\components\Stripe\Service\AbstractService
{
    /**
     ...
     *
     * @return \app\components\Stripe\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \app\components\Stripe\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \app\components\Stripe\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \app\components\Stripe\StripeClient([clientId => <CLIENT-ID>
     ...
     *
     * @return \app\components\Stripe\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \app\components\Stripe\Service\OAuthService from \app\components\Stripe\OAuth, where `connect_base`
                // was the name of the parameter that behaves as `api_base` does here.
                throw new \app\components\Stripe\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \app\components\Stripe\Util\RequestOptions::parse($opts);
    }
